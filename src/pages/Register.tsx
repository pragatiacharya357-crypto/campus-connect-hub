...
const STEPS = ["Email Verification", "ID Card Upload", "Profile Setup"];
...
const Register = () => {
  ...
  const handleEmailSubmit = async (e: React.FormEvent) => {
    ...
    if (!email.endsWith(".edu") && !email.includes(".edu.")) {
      toast.error("Please use your official university email (.edu)");
      return;
    }
    setEmailSent(true);
  ...
  const handleProfileSetup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Update profile with username, display name, hashed ID, and anonymous preference
      const { error: profileError } = await supabase
        .from("profiles")
        .update({
          username,
          display_name: displayName || null,
          hashed_id_number: idNumber, // In production, hash this server-side
      ...
      >
        ...
        <Card className="border-0 shadow-2xl">
          <CardHeader className="text-center">
            ...
            <CardDescription>
              {step === 0 && "Enter your university email to get started"}
